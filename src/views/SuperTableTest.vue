...
<template>
	<div class="tables-demo">
		...
		<section>
			...
			<BaseTable :data="users" :columns="userColumns" :loading="loadingUsers" :rows="10">
				<template #status="{ data }">
					<span class="status-pill" :class="data.status === 'Activo' ? 'active' : 'inactive'">
						{{ data.status }}
					</span>
				</template>
				...
				<template #actions="{ data }">
					<div class="actions">
						<Button icon="pi pi-pencil" text rounded @click="editRow(data)" />
						<Button icon="pi pi-trash" text rounded severity="danger" @click="deleteRow(data)" />
					</div>
				</template>
			</BaseTable>
		</section>
		...
		<section>
			...
			<BaseTable :data="claims" :columns="claimColumns" :loading="loadingClaims" :rows="15">
				...
				<template #actions="{ data }">
					<div class="actions">
						<BaseButton label="Ver" icon="pi pi-eye" text rounded />
					</div>
				</template>
			</BaseTable>
		</section>
