...
<template>
	<div class="super-table-wrapper">
		<DataTable :value="loading ? Array.from({ length: rows }) : data" :paginator="showPagination" :rows="rows"
			:rowsPerPageOptions="rowsPerPageOptions" :loading="loading" scrollable scrollHeight="flex"
			tableStyle="min-width: 80rem" stripedRows class="custom-table"
			...
			<template #header>
				<div class="table-header">
					<MultiSelect v-model="selectedColumns" :options="columns" optionLabel="header" display="chip"
						placeholder="Columnas visibles" class="column-selector" />
				</div>
			</template>
			...
			<Column v-for="col in selectedColumns" :key="col.field" :field="col.field" :header="col.header"
				:sortable="col.sortable" :style="{ width: col.width, textAlign: col.align || 'left' }">
				<template #body="slotProps">
					<!-- SKELETON -->
					<Skeleton v-if="loading" height="1rem" borderRadius="6px" />

					<!-- DATA -->
					<slot v-else :name="col.field" :data="slotProps.data" :field="col.field">
